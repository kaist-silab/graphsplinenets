...
class osc1d(object):
    ...
    def _continuous(self, x: float, k: int, n: int, r: int) -> tuple[torch.tensor, torch.tensor]:
...
class osc2d(object):
    ...
    def __init__(self, p, c, z, device):
        ...
        self.para = torch.linalg.solve(self.weight, self.value).reshape(((self.nx-1)*(self.r-1)+2, -1)).to(device)
    ...
    def _generator2d(self, p, c, z, nx, ny, r):
        ...
        for x_part, y_part, x_coll, y_coll in product(range(nx), range(ny), range(r-1), range(r-1)):
            ...
            coll_x = coll[0]
            coll_y = coll[1]
            for x_base_idx, y_base_idx in product(range(nx * (r - 1)), range(ny * (r - 1))):
                weight[x_base_idx, y_base_idx] += self.x_base[x_base_idx].f(coll_x) * self.y_base[y_base_idx].f(coll_y)
