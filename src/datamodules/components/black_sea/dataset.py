...
class BlackSeaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        xs = []
        for idx_ in range(time_index, time_index + self.context_steps + self.target_steps):
            x = np.stack(
                [
    ...
    def __repr__(self):
        ...
        for column in columns: table.add_column(column, justify="center", style="cyan") 
        table.add_row(fromtimestamp(self.data_files[0]['time'][0]), fromtimestamp(self.data_files[0]['time'][-1]), 
                        '\n'.join(str(d) for d in self.keys), str(self.context_steps), str(self.target_steps))
        console = Console()
