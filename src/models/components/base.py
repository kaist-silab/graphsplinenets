...
class BaseGNN(LightningModule):
    ...
    def instantiate_model(self):
        ...
        self.encoder = hydra.utils.instantiate(
            self.model_cfg.encoder, _recursive_=False
        )
        log.info(f"Instantiating encoder <{self.model_cfg.decoder._target_}>")
        self.decoder = hydra.utils.instantiate(
            self.model_cfg.decoder, _recursive_=False
        )
        log.info(f"Instantiating encoder <{self.model_cfg.node_update._target_}>")
        self.node_update = hydra.utils.instantiate(
            self.model_cfg.node_update, _recursive_=False
        )
        log.info(f"Instantiating encoder <{self.model_cfg.edge_update._target_}>")
        self.edge_update = hydra.utils.instantiate(
            self.model_cfg.edge_update, _recursive_=False
        )
    ...
    def softmax(self, g, node_feature, edge_feature):
        ...
        g.update_all(
            message_func=dgl.function.copy_e("wh", "m"),
    ...
    def edge_update_func(self, edges):
        ...
        edge_model_input = torch.cat(
            [edge_feature, src_node_feature, dis_node_feature], dim=-1
        )
