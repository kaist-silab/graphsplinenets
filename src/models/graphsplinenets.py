...
class SpaceSplineNets(BaseGNN):
    ...
    def training_step(self, batch, batch_nb): 
        ...
        out_feat_int = interpolate(out_feat, size=(8, 8), mode='bilinear', align_corners=False)
...
class TimeSpaceSplineNets(BaseGNN):
    ...
    def _get_base(self, nx, nx_sim, dx):
        ...
        base_start = torch.cat((x[:split_x] * (sx[1] - x[:split_x]) ** 2 / dx ** 2, torch.zeros(nx_sim - split_x)))
        base_end = torch.cat((torch.zeros(nx_sim - split_x), (x[-split_x:] - sx[-1]) * (x[-split_x:] - sx[-2]) ** 2 / dx ** 2))
